...
def load_data(src_dir, name):
    ...
    def read_hdf(src_dir, name, n_channel):
        df = pd.read_hdf(os.path.join(
            src_dir, "{}_ch{}.h5".format(name, n_channel)))
...
def plot_one_day_4ch(date, df_ch1, df_ch2, df_ch3, df_ch4, signal_filt=False, excess_attenuation=False, bool_save=False, show=False):
    ...
    df_ch1 = df_ch1[(df_ch1.index >= date) & (
        df_ch1.index < (date+timedelta(days=1)))]
    df_ch2 = df_ch2[(df_ch2.index >= date) & (
        df_ch2.index < (date+timedelta(days=1)))]
    df_ch3 = df_ch3[(df_ch3.index >= date) & (
        df_ch3.index < (date+timedelta(days=1)))]
    df_ch4 = df_ch4[(df_ch4.index >= date) & (
        df_ch4.index < (date+timedelta(days=1)))]
    ...
    for i in range(0, 4):
        ...
        if not excess_attenuation:
            ...
            if signal_filt:
                axs[i % 2, i//2].plot(time, df['signal_filt'],
                                      color="darkblue")
            axs[i % 2, i//2].grid()
            axs[i % 2, i//2].set_ylim(-50, 20)
            axs[i % 2, i//2].set_ylabel("Power [dB]")
            axs[i % 2, i //
                2].xaxis.set_major_formatter(mdates.DateFormatter('%H:%M'))
        else:
            axs[i % 2, i//2].plot(time,
                                  df['excess_attenuation'], color='lightblue')
            axs[i % 2, i//2].plot(time, np.zeros_like(time,
                                  dtype=float), color='red')
            axs[i % 2, i//2].grid()
            axs[i % 2, i//2].set_ylabel("Power [dB]")
            axs[i % 2, i //
                2].xaxis.set_major_formatter(mdates.DateFormatter('%H:%M'))
        ...
        if df.loc[:, 'flag'][0] == 1:  # if rain at the start of the day
            idx_start.append(0)
        # find index of 0 to 1 transitions in flags
        for idx in np.where(np.diff(df.loc[:, 'flag'].values) == True)[0]:
            idx_start.append(idx)
        # find index of 1 to 0 transitions in flags
        for idx in np.where(-np.diff(df.loc[:, 'flag'].values) == True)[0]:
            idx_stop.append(idx)
        ...
        for k in range(0, len(idx_start)):
            axs[i % 2, i//2].axvspan(df.index[idx_start[k]],
                                     df.index[idx_stop[k]], color='red', alpha=0.25)
        ...
        if flag_failure[0] == 1:
            idx_start.append(0)
        # find index of 0 to 2 transitions in flags
        for idx in np.where(np.diff(flag_failure) == True)[0]:
            idx_start.append(idx)
        # find index of 2 to 0 transitions in flags
        for idx in np.where(-np.diff(flag_failure) == True)[0]:
            idx_stop.append(idx)
        if flag_failure[-1] == 1:
        ...
        for k in range(0, len(idx_start)):
            axs[i % 2, i//2].axvspan(df.index[idx_start[k]],
                                     df.index[idx_stop[k]], color='grey', alpha=0.25)

    fig.suptitle("Alphasat received data at LLN for " +
...
def plot_RAPIDS_outputs(rapids_data, title, link_budget=False):
    """
    Plot RAPIDS attenuation from the rapids_data dataframe. The dataframe must contain a "PROBABILITY" and a "ATTENUATION" field.
    """
    plt.figure()
    if link_budget:
        plt.semilogx(rapids_data["PROBABILITY"].values,
                     rapids_data["ATTENUATION"].values, label='Link Budget')
    else:
        for i in range(0, 18):
            plt.semilogx(rapids_data["PROBABILITY"].values[i*26:(i+1)*26],
                         rapids_data["ATTENUATION"].values[i*26:(i+1)*26], label="{}°".format((i+1)*5))
        plt.xlim(1e-3, 1e2)
    plt.grid()
    plt.suptitle(title)
    plt.legend()
